...
- Windows 发行物：`apps/desktop/release5/` —— **NSIS 安装器 METIS Academy Setup 1.0.0.exe（92.8 MB）** + portable zip（125.6 MB）+ win-unpacked；全部为 06:54 终版构建（含音效+xterm+全部修复）
...
- 发行物：portable zip（125,630,919 字节，07:45 终版）+ win-unpacked 目录，asar 内含 dist/main/main.js、dist/preload/index.js、dist/renderer/index.html（@electron/asar 校验 OK）
