...
- Name: app.e2e.spec.ts >> Title Screen 显示标题与菜单
- Location: tests\e2e\app.e2e.spec.ts:49:5
...
Locator: getByText('METIS Academy')
Expected: visible
Error: strict mode violation: getByText('METIS Academy') resolved to 2 elements:
    1) <h1 class="text-5xl font-bold tracking-tight">METIS Academy</h1> aka getByRole('heading', { name: 'METIS Academy' })
    2) <p>© 2026 METIS Academy · Open Source (MIT)</p> aka getByText('© 2026 METIS Academy · Open')

Call log:
  - Expect "toBeVisible" getByText('METIS Academy') with timeout 5000ms
  - waiting for getByText('METIS Academy')
