...
## 7. NSIS 重打包 AV 竞态（不阻断；release-final 已可用）
- release-final（03:08 构建）已通过安装→启动→卸载全链路实测。
- 后续代码变更（2 个创业线决策事件 + relatedSkillIds 接线）尚未纳入安装包。
- 重打包方法：关闭所有 METIS 进程后运行 pnpm dist；若遇 UNKNOWN 错误等 2 分钟后重试（Windows Defender 实时扫描竞态）。
